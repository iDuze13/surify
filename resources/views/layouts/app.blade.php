...
<head>
    ...
    <style>
        ...
        body { background-color: var(--bg-body); color: var(--text-body); font-family: 'Outfit', sans-serif; transition: background-color 0.2s ease, color 0.2s ease; }
        ...
        .dark #lang-dropdown { background: #1e293b; border-color: #334155; }
        ...
        .dark .lang-option { color: #cbd5e1; }
        .lang-option:hover { background: #f0f7ff; color: #28628f; }
        .dark .lang-option:hover { background: #334155; color: #97ccfe; }
        .lang-option.active { background: #e8f3fb; color: #28628f; }
        .dark .lang-option.active { background: #1e3a52; color: #97ccfe; }
        ...
        @media (min-width: 1024px) {
            #main-sidebar { transform: translateX(0); position: sticky; top: 0; height: 100vh; }
        }
        ...
        .side-divider { height: 1px; background: #e2e8f0; margin: 8px 4px; }
        .dark .side-divider { background: #334155; }
    </style>
    <script>
...
<body class="antialiased">
    ...
    <div class="flex min-h-screen">
        ...
        <aside id="main-sidebar" class="fixed lg:sticky inset-y-0 left-0 z-40 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-700 flex flex-col shrink-0">
            ...
            <nav class="flex-1 overflow-y-auto p-3 flex flex-col gap-1">
                ...
                <a href="{{ route('dashboard') }}" class="side-link {{ request()->routeIs('dashboard') ? 'active' : '' }}"><span class="material-symbols-outlined text-[18px]">dashboard</span> Dashboard</a>
                @endif

                @if($esAdmin)
                <a href="{{ route('admin.roles.index') }}" class="side-link {{ request()->routeIs('admin.roles.*') ? 'active' : '' }}"><span class="material-symbols-outlined text-[18px]">admin_panel_settings</span> Roles</a>
                <a href="{{ route('admin.usuarios.index') }}" class="side-link {{ request()->routeIs('admin.usuarios.*') ? 'active' : '' }}"><span class="material-symbols-outlined text-[18px]">group</span> Usuarios</a>
                @endif

                @if($tieneAlguno(['crear_destino', 'modificar_destino', 'eliminar_destino', 'administrar_destinos_sugeridos']))
                <a href="{{ route('admin.destinos.index') }}" class="side-link {{ request()->routeIs('admin.destinos.*') ? 'active' : '' }}"><span class="material-symbols-outlined text-[18px]">landscape</span> Destinos</a>
                @endif

                @if($tieneAlguno(['crear_evento', 'modificar_evento', 'eliminar_evento', 'administrar_eventos_sugeridos']))
                <a href="/admin/eventos" class="side-link"><span class="material-symbols-outlined text-[18px]">celebration</span> Festivales</a>
                @endif

                @if($tieneAlguno(['administrar_resena', 'administrar_resenas', 'eliminar_comentario']))
                <a href="{{ route('admin.resenas.index') }}" class="side-link {{ request()->routeIs('admin.resenas.*') ? 'active' : '' }}"><span class="material-symbols-outlined text-[18px]">chat_bubble</span> Reseñas</a>
                @endif

                @if($tieneAlguno(['gestionar_gastronomia']))
                <a href="{{ route('admin.gastronomia.index') }}" class="side-link {{ request()->routeIs('admin.gastronomia.*') ? 'active' : '' }}"><span class="material-symbols-outlined text-[18px]">restaurant</span> Gastronomía</a>
                @endif
        ...
        <div class="flex-1 flex flex-col min-w-0">
            ...
            <header class="border-b border-slate-200 bg-white/90 backdrop-blur-md sticky top-0 z-20 shadow-sm dark:border-slate-700 dark:bg-slate-900/90">
                <div class="px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">
                    ...
                    <button onclick="toggleSidebar(true)" class="lg:hidden w-9 h-9 flex items-center justify-center text-slate-600 dark:text-slate-300 hover:text-[#28628f] bg-slate-100 dark:bg-slate-800 rounded-lg transition-colors shrink-0">
                    ...
                    <div class="relative flex-grow max-w-md hidden md:block" id="search-container">
                        <div class="flex items-center bg-slate-100 border border-transparent rounded-full px-4 py-2 gap-2 focus-within:border-[#28628f] focus-within:bg-white transition-all dark:bg-slate-800">
                            ...
                            <input id="search-input" type="text" placeholder="Destinos, festivales, provincias..."
                                class="bg-transparent border-none p-0 focus:ring-0 text-sm text-slate-700 placeholder:text-slate-400 w-full dark:text-slate-200" autocomplete="off">
                        </div>
